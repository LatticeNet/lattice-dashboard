...
export default {
  common: {
    ...
    // The one node status vocabulary (lattice-server node_status.go). Every
    // fleet surface prints these five words and nothing else for a node.
    nodeStatus: {
      online: "online",
      degraded: "degraded",
      offline: "offline",
    ...
    nodeStatusHint: {
      online: "Reporting within the last 90 seconds and nothing proven broken.",
      degraded: "Reporting, but a probe on the node says something is broken.",
      offline: "Stopped reporting. The control plane stops trusting a node after 90 seconds of silence.",
      neverReported: "Enrolled, but no report has ever arrived. The agent was never installed or never reached the control plane.",
      disabled: "Switched off by an operator. The agent token is refused until the node is enabled again.",
    },
    status: {
      neverReported: "never reported",
      active: "active",
  ...
  overview: {
    ...
    acrossLive: "across {count} reporting nodes with metrics",
    summary: {
      download: "Download",
      upload: "Upload",
      regions: "Regions",
    ...
    fleetAllHealthy: "Every node is online",
    fleetAllHealthyHint: "{count} nodes online, none degraded, offline, disabled, or never reported.",
    fleetNeedsAttention: "Needs attention",
    fleetHealthyCount: "{count} online",
